...
//    var tokenStorage = OAuth2TokenStorage()
    ...
    func fetchProfileImageURL(_ username: String, completion: @escaping (Result<String, Error>) -> Void) {
        ...
        if lastToken == tokenStorage.getBearerToken() { return }
        task?.cancel()
        lastToken = tokenStorage.getBearerToken()
...
/*
 ...
 private var lastToken: String?
 private var lastUsername: String?
 private let token = SwiftKeychainWrapper.standard.getAuthToken()
 var tokenStorage = SwiftKeychainWrapper()
     ...
     let task = urlSession.objectTask(for: request) { (result: Result<UserResult, Error>) in
                 ...
                 case .success(let json):
                     DispatchQueue.main.async {
                         ...
                         completion(.success(json.avatarURL))
